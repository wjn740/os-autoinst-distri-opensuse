...
#	zypper_call("in --replacefiles perl-DBD-SQLite");
	...
	script_run('while line=$(nc -l 12345); do if [ "X$line" == "XDone" ]; then echo $line;break; else echo $line; fi done', 7200);

        script_run('systemctl stop openqa-worker@1');
        script_run('systemctl stop openqa-worker@2');
        script_run('systemctl stop openqa-worker@3');
        script_run('systemctl stop openqa-worker@4');
        script_run('systemctl stop openqa-worker@5');
        script_run('systemctl stop openqa-worker@6');
        script_run('systemctl stop openqa-worker@7');
        script_run('systemctl stop openqa-worker@8');
}
