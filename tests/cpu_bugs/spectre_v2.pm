...
"cpuid -1 -l 7 -s 0 -r | awk \'{print \$6}\' | awk -F \"=\" \'{print \$2}\' | tail -n1"
    ...
    if ( hex $edx & 0x04000000 ) {
        assert_script_run(
"(echo \"Ucode has been update for IBRS and IBPB support\") | tee /dev/$serialdev"
        );
        record_info( 'ok', "Hardware support IBRS and IBPB" );
        $supported = 1;
    }
    else {
        record_info( 'softfail', "Hardware doesn't support IBRS and IBPB" );
        $supported = 0;
    }
...
'dmesg | grep "Enabling Restricted Speculation for firmware calls"'
        ...
        record_info( 'suppported IBRS mode',
            "spectre_v2 default mode finish." );
...
'dmesg | grep -v "Enabling Restricted Speculation for firmware calls"'
    ...
    if ( $cpu_family == 6 ) {
        assert_script_run(
'dmesg | grep "retpoline,amd selected but CPU is not AMD. Switching to AUTO select"'
        );
        record_info( 'PASS',
            "using AUTO mode due to this is not a AMD machine." );
    }
    else {
        assert_script_run(
            'dmesg | grep "retpoline,amd selected on command line."');
        assert_script_run( 'cat ' . $syspath . 'spectre_v2' );
        assert_script_run( 'cat '
              . $syspath
              . 'spectre_v2'
              . '| grep "^Mitigation: Full AMD retpoline$"' );
