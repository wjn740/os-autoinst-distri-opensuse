...
sub run {
    if (get_var("MIGRATION_HOST")) {
        ...
        assert_script_run("sed -i \"/^" . "\\" . ${vm_pool} . "/d\" /etc/exports");
        assert_script_run("virsh start $name", fail_message => "You need run install testcase to setup a KVM guest for migration.");
#setup NFS and release a lock to notice client side
        zypper_call("in nfs-kernel-server");
        assert_script_run("echo \"$vm_pool    *(rw,sync,no_root_squash)\" >>/etc/exports");
...
#Do migrate
        assert_script_run("virsh migrate --live $name --verbose qemu+tcp://$dest_host/system");
...
#access this machine without password
        ...
        assert_script_run("sed -i 's/#listen_tcp = 1/listen_tcp = 1/g' /etc/libvirt/libvirtd.conf");
        assert_script_run("sed -i 's/#auth_tcp = .*/auth_tcp = \"none\"/g' /etc/libvirt/libvirtd.conf");
